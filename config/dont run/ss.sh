...
echo
echo -e "${cyan}Link --------> ${blanco}${link}"
echo
echo -e "${amarillo}Si no se genero el enlance"
read
ps
echo
...
fuistehackeado() {
banner
python -m http.server 8084 > /dev/null 2>&1 &
echo
curl https://raw.githubusercontent.com/WhBeatZ/hacked/main/i.png -o ip.png
curl https://github.com/WhBeatZ/hacked/blob/main/hacked.html -o hacked.html
echo
...
case $caca in
		Y | y | yes)
		am start -a android.intent.action.VIEW -d localhost:8084/hacked.html 2>/dev/null
		;;
